...
class EnergyDataset:
    ...
    def process_trainset(self):
        for key, df in self.data.items():
            ...
            self.data[key] = self._melt_and_add_hour(df, dc.COLUMNS['GNODEB_CELL_BEAM'], column_name)
            self.data[key] = self._split_gnodeb_cell_beam(self.data[key])

        self.train_set = pd.concat([df[df.columns.difference(dc.CATEGORICAL_FEATURES)] 
                                    for df in self.data.values()], axis=1)
        
        columns_to_keep = [dc.COLUMNS['GNODEB_CELL_BEAM']] + dc.CATEGORICAL_FEATURES + dc.NUMERICAL_FEATURES
    ...
    def process_testset(self):
        ...
        test_ids = [(hour, gnodeb, cell, beam)
                    for hour in test_hours
                    for gnodeb in range(30)
                    for cell in range(3)
                    for beam in range(32)]

        self.test_set = pd.DataFrame(test_ids, columns=[dc.COLUMNS['HOUR']] + dc.CATEGORICAL_FEATURES)
    ...
    @staticmethod
    def _melt_and_add_hour(df: pd.DataFrame, var_name: str, value_name: str) -> pd.DataFrame:
    ...
    def _split_gnodeb_cell_beam(df: pd.DataFrame) -> pd.DataFrame:
        df[dc.CATEGORICAL_FEATURES] = df[dc.COLUMNS['GNODEB_CELL_BEAM']].str.split('_', expand=True)
