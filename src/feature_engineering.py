...
def apply_pca(
    full_data: pd.DataFrame, pca_features: List[str], n_components: int = 5
) -> pd.DataFrame:
    ...
    principal_df = pd.DataFrame(
        data=principal_components, columns=pca_columns, index=full_data.index
    )
...
def apply_groupby(full_data: pd.DataFrame) -> pd.DataFrame:
    ...
    full_data["mean_traffic_per_gnodeb"] = full_data.groupby(groupby_cols)[
        dc.TARGET_COLUMN
    ].transform("mean")
    full_data["std_traffic_per_gnodeb"] = full_data.groupby(groupby_cols)[
        dc.TARGET_COLUMN
    ].transform("std")
